...
const UpdateForm = ({userId}) => {
  ...
  const [receipt, setReceipt] = useState({
    ...
    const formatDate = dateColombia.locale('es').format('YYYY-MM-DD');
    return formatDate;
  };
  ...
  const loadReceipt = async () => {
    const receiptData = await axios.get(`${apiUrl}/findById/141`);

    try {
      setReceipt({
        receiptName: receiptData.data.receiptName,
        price: receiptData.data.price,
        amount: receiptData.data.amount,
        date: FormatDate(receiptData.data.date),
        typeService: {
          type: receiptData.data.typeService.type,
        },
        house: {
          name: receiptData.data.houseName,
        }
      });
      setReceiptType(receiptData.data.typeService.type);
      setSelectedHouse(receiptData.data.houseName);
    } catch (error) {
      console.log(error);
    }
  }
